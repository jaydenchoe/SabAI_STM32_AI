...
#ifndef __AI_CNN_HUMAN_H__
#define __AI_CNN_HUMAN_H__
#pragma once

#include "ai_platform.h"
#include "ai_platform_interface.h"

#define AI_CNN_HUMAN_MODEL_NAME          "cnn_human"

#define AI_CNN_HUMAN_ACTIVATIONS_ALIGNMENT   (4)

#define AI_CNN_HUMAN_IN_NUM       (1)
#define AI_CNN_HUMAN_IN { \
  AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_FLOAT, 90, 3, 1, 1, NULL), \
}
#define AI_CNN_HUMAN_IN_SIZE { \
  (90 * 3 * 1), \
}
#define AI_CNN_HUMAN_IN_1_SIZE  (90 * 3 * 1)
#define AI_CNN_HUMAN_IN_1_SIZE_BYTES  ((90 * 3 * 1) * 4)





#define AI_CNN_HUMAN_OUT_NUM      (1)
#define AI_CNN_HUMAN_OUT { \
  AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_FLOAT, 1, 1, 3, 1, NULL), \
}
#define AI_CNN_HUMAN_OUT_SIZE { \
  (1 * 1 * 3), \
}
#define AI_CNN_HUMAN_OUT_1_SIZE  (1 * 1 * 3)
#define AI_CNN_HUMAN_OUT_1_SIZE_BYTES  ((1 * 1 * 3) * 4)

#define AI_CNN_HUMAN_N_NODES (7)
 ...
 * @ref ai_cnn_human_create. Both the activations memory buffer 
 ...
 * using @ref ai_cnn_human_get_error
 */
AI_API_ENTRY
 ...
 * header generated by the code generation tool. See AI_CNN_HUMAN_*
 * defines into file @ref cnn_human.h for all network sizes defines
 *
 * @param network an opaque handle to the network context
 ...
 */
AI_API_ENTRY
ai_i32 ai_cnn_human_forward(
  ai_handle network, const ai_buffer* input);

AI_API_DECLARE_END

#endif /*__AI_CNN_HUMAN_H__*/
