...
#ifndef __AI_AXIS_SENSOR_H__
#define __AI_AXIS_SENSOR_H__
#pragma once

#include "ai_platform.h"
#include "ai_platform_interface.h"

#define AI_AXIS_SENSOR_MODEL_NAME          "axis_sensor"

#define AI_AXIS_SENSOR_ACTIVATIONS_ALIGNMENT   (4)

#define AI_AXIS_SENSOR_IN_NUM       (1)
#define AI_AXIS_SENSOR_IN { \
  AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_FLOAT, 128, 3, 1, 1, NULL), \
}
#define AI_AXIS_SENSOR_IN_SIZE { \
  (128 * 3 * 1), \
}
#define AI_AXIS_SENSOR_IN_1_SIZE  (128 * 3 * 1)
#define AI_AXIS_SENSOR_IN_1_SIZE_BYTES  ((128 * 3 * 1) * 4)





#define AI_AXIS_SENSOR_OUT_NUM      (1)
#define AI_AXIS_SENSOR_OUT { \
  AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_FLOAT, 1, 1, 4, 1, NULL), \
}
#define AI_AXIS_SENSOR_OUT_SIZE { \
  (1 * 1 * 4), \
}
#define AI_AXIS_SENSOR_OUT_1_SIZE  (1 * 1 * 4)
#define AI_AXIS_SENSOR_OUT_1_SIZE_BYTES  ((1 * 1 * 4) * 4)

#define AI_AXIS_SENSOR_N_NODES (6)

AI_API_DECLARE_BEGIN

/*!
 * @defgroup axis_sensor
 ...
 * @ref ai_axis_sensor_create. Both the activations memory buffer 
 ...
 * using @ref ai_axis_sensor_get_error
 */
AI_API_ENTRY
 ...
 * header generated by the code generation tool. See AI_AXIS_SENSOR_*
 * defines into file @ref axis_sensor.h for all network sizes defines
 *
 * @param network an opaque handle to the network context
 ...
 */
AI_API_ENTRY
ai_i32 ai_axis_sensor_forward(
  ai_handle network, const ai_buffer* input);

AI_API_DECLARE_END

#endif /*__AI_AXIS_SENSOR_H__*/
