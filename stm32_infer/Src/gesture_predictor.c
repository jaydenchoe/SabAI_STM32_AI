...
extern const int kWingGesture;
extern const int kRingGesture;
extern const int kSlopeGesture;
extern const int kNoGesture;

extern const float kDetectionThreshold;
extern const int kPredictionSuppressionDuration;


// State for the averaging algorithm we're using.
float prediction_history[4][K_PREDICTION_HISTORY_LENGTH];
...
int PredictGesture(float* output) {
  ...
  for (int i = 0; i < K_GESTURE_COUNT; ++i) {
  ...
  if (prediction_history_index >= K_PREDICTION_HISTORY_LENGTH) {
  ...
  for (int i = 0; i < K_GESTURE_COUNT; i++) {
    ...
    for (int j = 0; j < K_PREDICTION_HISTORY_LENGTH; ++j) {
      prediction_sum += prediction_history[i][j];
    }
    const float prediction_average = prediction_sum / K_PREDICTION_HISTORY_LENGTH;
