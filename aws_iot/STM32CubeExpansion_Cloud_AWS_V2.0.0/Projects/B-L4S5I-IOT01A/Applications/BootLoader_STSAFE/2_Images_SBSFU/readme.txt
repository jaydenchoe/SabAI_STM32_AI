...
application through the pkcs11 APIs (KEY IDbased APIs) that are executed inside
...
STSAFE-A1xx is a tamper-resistant secure element (HW Common Criteria EAL5+ certified)
used to host X509 certificates and keys, and to perform verifications that are used
for firmware image authentication during Secure Boot and Secure Firmware Update
procedures.
...
@par Directory contents
   ...
   - 2_Images_SBSFU/SBSFU/App/sfu_kms.c                       Key management service blob importation handling
   - 2_Images_SBSFU/SBSFU/App/sfu_interface_crypto_scheme.c   Crypto scheme implementation based on KMS API
   ...
   - 2_Images_SBSFU/SBSFU/App/sfu_scheme_x509_core.c          Core crypto functions used by the x509 crypto scheme
   - 2_Images_SBSFU/SBSFU/App/sfu_scheme_x509_crt.c           Certificate handling functions for x509 scheme
   - 2_Images_SBSFU/SBSFU/App/sfu_scheme_x509_if.c            Interface of SBSFU to x509 scheme
   - 2_Images_SBSFU/SBSFU/App/sfu_scheme_x509_mbedtls_ecdsa_verify.c Custom ecdsa_verify implementation for mbedtls (hands off to stsafe via PKCS11)
   ...
   - 2_Images_SBSFU/SBSFU/App/sfu_kms.h                       Header file for sfu_kms.c
   - 2_Images_SBSFU/SBSFU/App/sfu_interface_crypto_scheme.h   Header file for sfu_interface_crypto_scheme.h
   ...
   - 2_Images_SBSFU/SBSFU/App/sfu_scheme_x509_config.h        X509 scheme configuration file
   - 2_Images_SBSFU/SBSFU/App/sfu_scheme_x509_config_mbedtls.h mbedtls configuration file for x509 scheme
   - 2_Images_SBSFU/SBSFU/App/sfu_scheme_x509_core.h          Header file for sfu_scheme_x509_core.c
   - 2_Images_SBSFU/SBSFU/App/sfu_scheme_x509_crt.h           Header file for sfu_scheme_x509_crt.c
   - 2_Images_SBSFU/SBSFU/App/sfu_scheme_x509_embedded_certs.h SBSFU embedded certificates to replace STSAFE in case provisioning is not used
...
1. Compile projects in the following order. This is mandatory as each project requests some objects
  ...
  - STSAFE_Provisioning (run once to provision the STSAFE)
...
8. Send the firmware image prepared by the AWS project (please refer to the AWS project readme for further information).
   By default this image will be installed and set in "self test" state.
   The application must be self-validated before the next reset otherwise it will be uninstalled by SBSFU.
