...
/** @defgroup SFU_CONFIG_OPT_FEATURES Optional Features Software Configuration
  * @{
  */
#define SECBOOT_LOADER SECBOOT_USE_LOCAL_LOADER    /*!< Loader selection inside SBSFU : local/external/none */

#define SECBOOT_USE_LOCAL_LOADER              (1U) /*!< local loader feature integrated into SBSFU (YMODEM over UART) */
#define SECBOOT_USE_EXTERNAL_LOADER           (2U) /*!< external loader : see specific loader project  */
...
/* Uncomment the switch below to bypass the check of the active slot: check if no malicious code beyond FW size */
#define SFU_BYPASS_ACTIVE_SLOT_VERIF
...
/*
 * The watchdog is enabled by default because aws_tests requires a watchdog for self-tests failure detection.
 * You need to comment out this define if you want to use a debugger.
 */
...
#define SFU_MPU_USERAPP_ACTIVATION /*!< MPU protection during UserApp execution : Only Slot0 considered as an
                                        exectuable area */
...
#undef SFU_BYPASS_ACTIVE_SLOT_VERIF              /*! check there is no malicious code at the end of the active slot */
...
/** @defgroup SFU_CONFIG_IWDG IWDG Configuration
  * @{
  */
#define SFU_IWDG_TIMEOUT  ((uint32_t)32) /*!< IWDG timeout in seconds (the max. value that can be set here depends on
