...
/** @addtogroup SFU Secure Boot / Secure Firmware Update
  * @{
  */
extern uint8_t gRollbackDone; /* indicates if a rollback has been performed : see sfu_fwimg_services.c */
...
#if (SECBOOT_LOADER == SECBOOT_USE_LOCAL_LOADER) || (SECBOOT_LOADER == SECBOOT_USE_EXTERNAL_LOADER)
static void SFU_BOOT_SM_CheckNewFwToDownload(void);
static void SFU_BOOT_SM_DownloadNewUserFw(void);
#endif /* (SECBOOT_LOADER == SECBOOT_USE_LOCAL_LOADER) || (SECBOOT_LOADER == SECBOOT_USE_EXTERNAL_LOADER) */
#ifdef KMS_ENABLED
...
static SFU_ErrorStatus SFU_BOOT_Init(void)
{
  ...
  else
  {
    ...
    if (e_se_status == SE_BOOT_INFO_ERR_FACTORY_RESET)
    {
      ...
      /* Please add your custom action here : let's consider it as a success to avoid a useless reboot */
...
static void SFU_BOOT_SM_CheckStatusOnReset(void)
{
  ...
  SFU_SET_SM_IF_CURR_STATE(e_ret_status, SFU_STATE_CHECK_KMS_BLOB_TO_INSTALL, SFU_STATE_HANDLE_CRITICAL_FAILURE);
#else
...
/**
  * @brief  Check if a KMSBlob is waiting for instal.
  *         When entering this state from SFU_STATE_CHECK_STATUS_ON_RESET (initialDeviceStatusCheck=1) it is required
  *         to press the user button to force the local download.
  *         When entering this state from SFU_STATE_VERIFY_USER_FW_STATUS the local download is awaited automatically
  *         (because there is no other action to do).
...
{
  SFU_ErrorStatus  e_ret_status = SFU_ERROR;
  uint8_t         *pBlobToImport;
  CK_ULONG        ImportBlobResult;
  CK_RV           e_status;
  ...
  /* Initialize KMS in order to be able to import blob using internal algorythms */
...
#error "The current example does not support the selected Firmware Image Programming type."
    ...
    case SFU_IMG_NO_FWUPDATE:
      ...
      {
#ifdef ENABLE_IMAGE_STATE_HANDLING
        /* we could have an interrupted rollback
         * the slot 0 header is valid (with src image fingerprint != 0) and a non valid fw image (the old image partially rolled back)
         * we attempt to continue the rollback
         */
...
#if (SECBOOT_LOADER == SECBOOT_USE_EXTERNAL_LOADER)
/**
...
#endif /* SECBOOT_LOADER == SECBOOT_USE_EXTERNAL_LOADER */
...
static void SFU_BOOT_SM_VerifyUserFwSignature(void)
{
  ...
  if(SFU_SUCCESS == e_ret_status)
  {
    ...
    else
    {
      ...
      if (0 == gRollbackDone)
      {
    	  /* avoid an infinite loop of reboot : local download state will be entered on next boot */
    	  (void)SFU_IMG_InvalidateCurrentFirmware(); /* If this fails we continue anyhow */
      }
      /* else : reboot and check rolled-backed image */
    }
  }
#endif /* ENABLE_IMAGE_STATE_HANDLING */
