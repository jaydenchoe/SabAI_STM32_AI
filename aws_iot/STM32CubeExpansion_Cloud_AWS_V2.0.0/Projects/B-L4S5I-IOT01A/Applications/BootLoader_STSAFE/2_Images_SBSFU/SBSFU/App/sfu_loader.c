...
SFU_ErrorStatus SFU_COM_YMODEM_DataPktRxCpltCallback(uint8_t *pData, uint32_t uSize)
{
  ...
  if (m_uPacketsReceived == 1)
  {
    ...
    if (e_ret_status == SFU_SUCCESS)
    {
      ...
      else
      {
    	...
    	/* Force the image state to VALID as we are in a local download case */
    	SE_FwRawHeaderTypeDef* pHdr = (SE_FwRawHeaderTypeDef*)pData;
  ...
  if ((m_uDwlAreaAddress < (m_uDwlAreaStart + SFU_IMG_IMAGE_OFFSET)) &&
      ((m_uDwlAreaAddress + uSize) >= (m_uDwlAreaStart + SFU_IMG_IMAGE_OFFSET)))
  {
