...
SFU_ErrorStatus SFU_LL_SECU_SetProtectionMPU_UserApp(void)
{
  ...
  MPU_InitStruct.BaseAddress          = APP_PROTECT_MPU_AREA_SLOT0_START;
  MPU_InitStruct.Size                 = APP_PROTECT_MPU_AREA_SLOT0_SIZE;
  MPU_InitStruct.SubRegionDisable     = APP_PROTECT_MPU_AREA_SLOT0_SREG;
  MPU_InitStruct.AccessPermission     = APP_PROTECT_MPU_AREA_SLOT0_PERM;
  MPU_InitStruct.DisableExec          = APP_PROTECT_MPU_AREA_SLOT0_EXEC;
  ...
  MPU_InitStruct.BaseAddress          = APP_PROTECT_MPU_AREA_EXT_START;
  MPU_InitStruct.Size                 = APP_PROTECT_MPU_AREA_EXT_SIZE;
  MPU_InitStruct.SubRegionDisable     = APP_PROTECT_MPU_AREA_EXT_SREG;
  MPU_InitStruct.AccessPermission     = APP_PROTECT_MPU_AREA_EXT_PERM;
  MPU_InitStruct.DisableExec          = APP_PROTECT_MPU_AREA_EXT_EXEC;
  MPU_InitStruct.IsShareable          = MPU_ACCESS_NOT_SHAREABLE;
  MPU_InitStruct.IsBufferable         = MPU_ACCESS_BUFFERABLE;
  MPU_InitStruct.IsCacheable          = MPU_ACCESS_CACHEABLE;
  MPU_InitStruct.TypeExtField         = MPU_TEX_LEVEL0;
...
SFU_ErrorStatus SFU_LL_SECU_SetProtectionMPU(void)
{
  ...
  /**
    ...
    *   - Region 1: size up to 256kB with up to 8*32kB sub-regions
    *   - Region 2: size up to 32kB  with up to 8*4kB sub-regions
    * Region 2 is placed right after Region 1, even if Region 1 is smaller than 128kB.
    *
    *
    * IMPORTANT: the maximum executable size is NOT 256kB + 32kB.
    *            The algorithm accepts a size up to 256kB + 32kB - 1 byte.
    *
    * Examples:
    *
    * 1. Region 1 is 256kB long with Region 2 being 8kB long
    * <--------------256kB---------------><-------32kB--------->
    * [Reg.1.1][Reg.1.2].........[Reg.1.8][Reg.2.1][Reg.2.2].FF.   (.FF. means deactivated)
    *
    * 2. Region 1 is 128kB long with Region 2 being 12kB long
    * <---------------256kB---------------><------------32kB------------->
    * [Reg.1.1][Reg.1.2].........[Reg.1.4][Reg.2.1][Reg.2.2][Reg.2.3].FF.   (.FF. means deactivated)
    *
    * The size covered by region 1 corresponds to the quotient of the sbsfu code size divided by 32kB.
    * The size covered by region 2 corresponds to the remainder of the sbsfu code size divided by 32kB, rounded-up to
    * the next 4kB.
    */
  uint32_t region1_first_32kb_sub_region_disabled; /* index of the first sub-region to be disabled in region 1 */
  uint32_t region2_first_4kb_sub_region_disabled;  /* index of the first sub-region to be disabled in region 2 */
  /* The total size of the executable code is: SFU_AREA_ADDR_END - FLASH_BASE + 1 */
  uint32_t sbsfu_code_size = (SFU_AREA_ADDR_END - FLASH_BASE + 1); /* sbsfu, secure engine, callgate, se_interface,
                                                                      PCROPed keys... */
  ...
  /* Compute the number of 16Kbytes sub-regions required to cover the executable size in Region 1 (up to 8 maximum).
    ...
    * The total size of the executable code is: SFU_AREA_ADDR_END - FLASH_BASE + 1 (sbsfu, secure engine, callgate,
    * se_interface, PCROPed keys...)
    *
    * WATCH OUT: This value cannot exceed SFU_PROTECT_MPU_MAX_NB_SUBREG (8 sub-regions).
    */
  region1_first_32kb_sub_region_disabled = sbsfu_code_size >> 15 ; /* shifting by 15 to divide by 32k */

  /* Compute the remainder of the code size divided by 16k */
  region2_first_4kb_sub_region_disabled = sbsfu_code_size & 32767;

  /* Compute the number of 4Kbytes sub-regions required to cover the end of the executable in Region 2 (up to 8 maximum)
    * This number of sub-regions is the left-SHIFT value we apply to decide which regions are disabled.
    */
  if (region2_first_4kb_sub_region_disabled != 0)
  {
    /*
      * The code size is not a multiple of 16k.
      * We compute the number of 4kB sub-regions needed in Region 2.
      *
      * +4095 to round-up to next 4k and shifting by 12 to divide by 4k
      */
    region2_first_4kb_sub_region_disabled = ((region2_first_4kb_sub_region_disabled + 4095) >> 12);
  }
  else
  {
    /*
      * No remainder: the code size is a multiple of 32k.
      * 2 possibilities but only 1 is supported:
      *  - All 4kB sub-regions are needed in Region 2 if the executable size is 256kB + 32kB.
      *    In this case, region1_first_32kb_sub_region_disabled reaches 9 so exceeds 8 and we return an error.
      *    NOT SUPPORTED
      *
      *  - Or no 4Kb sub-region is needed in Region 2 because the executable is contained fully in Region 1.
      *    SUPPORTED
      */
    region2_first_4kb_sub_region_disabled = 0; /* Will result in 0xFF << 0 : disable all sub-regions */
  }
  /*
    * If the executable size is more than 256kB + 32kB - 1 byte then we return an error,
    * because the algorithm would try configuring 9 sub-regions of 32kB.
    * Restriction: the current algorithm cannot compute that we need 8*32kB + 8*4kB.
    */
  if (region1_first_32kb_sub_region_disabled <= SFU_PROTECT_MPU_MAX_NB_SUBREG)
  {
    e_ret_status = SFU_SUCCESS;

#if defined(SFU_VERBOSE_DEBUG_MODE)
    TRACE("\r\n\t  [MPU] Region 1 with %d sub-region(s).", region1_first_32kb_sub_region_disabled);
    TRACE("\r\n\t  [MPU] Region 2 with %d sub-region(s).", region2_first_4kb_sub_region_disabled);
#endif /* SFU_VERBOSE_DEBUG_MODE */

    /* Sanity check: let's make sure the MPU settings cover the executable code */
    if (((region1_first_32kb_sub_region_disabled * 32768) + (region2_first_4kb_sub_region_disabled * 4096)) <
        sbsfu_code_size)
    {
    ...
    for (mpu_region_num = 0U; mpu_region_num < (sizeof(MpuAreas) / sizeof(SFU_MPU_InitTypeDef)); mpu_region_num++)
    {
      ...
      if (mpu_region_num == 1)
      {
        /* Region 1 (executable, up to 256kB with up to 8*32kB sub-regions) */
        MPU_InitStruct.SubRegionDisable = (uint8_t)(0xff << region1_first_32kb_sub_region_disabled);
      }
      else if (mpu_region_num == 2)
      {
        /* Region 2 (executable, up to 32kB with up to 8*4kB sub-regions) */
        MPU_InitStruct.SubRegionDisable = (uint8_t)(0xff << region2_first_4kb_sub_region_disabled);
        /* The region start address is the 16k-aligned address before the end address of the executable area */
        MPU_InitStruct.BaseAddress = (SFU_AREA_ADDR_END  & ~32767); /* 32k LSBs set to 0 */
      }
      else
      {
  ...
  {
    TRACE("\r\n\t  [MPU] Too many MPU sub-regions in Region 1 : %d > %d", region1_first_32kb_sub_region_disabled,
...
SFU_ErrorStatus SFU_LL_SECU_CheckProtectionPCROP(FLASH_OBProgramInitTypeDef *psFlashOptionBytes)
{
  ...
  if ((psFlashOptionBytes->PCROPStartAddr <= SFU_PROTECT_PCROP_ADDR_START)
      && (psFlashOptionBytes->PCROPEndAddr >= (SFU_PROTECT_PCROP_ADDR_END - 8U)))
  {
