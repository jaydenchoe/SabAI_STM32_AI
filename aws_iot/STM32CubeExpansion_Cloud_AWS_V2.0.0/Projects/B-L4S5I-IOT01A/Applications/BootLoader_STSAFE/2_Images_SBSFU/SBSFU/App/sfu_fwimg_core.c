...
#define CHUNK_SIZE_SIGN_VERIFICATION (2048U)  /*!< Signature verification chunk size*/
#else
...
#if (SECBOOT_CRYPTO_SCHEME == SECBOOT_X509_ECDSA_WITHOUT_ENCRYPT_SHA256)
#define SFU_IMG_CHUNK_SIZE  (2048U)
#else
...
/**
  * @brief  SFU_IMG Firmware Valid Type Definition
  */
typedef struct
{
  uint8_t Valid[3][MAGIC_LENGTH];
} SFU_FwValidTypeDef;
...
static SFU_ErrorStatus CheckMagic(uint8_t *pMagicAddr, uint8_t *pValidHeaderAddr, uint8_t *pTestHeaderAddr)
{
  ...
  SE_FwRawHeaderTypeDef* pFwHeader = NULL;
  void* pFwHdrMacPartial = NULL;
  ...
  {
    pFwHeader = (SE_FwRawHeaderTypeDef*) pValidHeaderAddr;
    pFwHdrMacPartial = (void*) (((uint32_t) pFwHeader->HeaderMAC) + sizeof(pFwHeader->HeaderMAC) - MAGIC_LENGTH / 2);
  ...
  {
    pFwHeader = (SE_FwRawHeaderTypeDef*) pTestHeaderAddr;
    pFwHdrMacPartial = (void*) (((uint32_t) pFwHeader->HeaderMAC) + sizeof(pFwHeader->HeaderMAC) - MAGIC_LENGTH / 2);
...
static SFU_ErrorStatus WriteMagic(uint8_t *pMagicAddr, uint8_t *pHeaderValid, uint8_t *pHeaderToInstall)
{
  ...
  SE_FwRawHeaderTypeDef* pFwHdrValid     = (SE_FwRawHeaderTypeDef*) pHeaderValid;
  SE_FwRawHeaderTypeDef* pFwHdrToInstall = (SE_FwRawHeaderTypeDef*) pHeaderToInstall;
  (void)SFU_LL_FLASH_Read(&magic[0],
                          (void*)((uint32_t)(pFwHdrValid->HeaderMAC) + SE_MAC_LEN - MAGIC_LENGTH / 2), MAGIC_LENGTH / 2);
  (void)SFU_LL_FLASH_Read(&magic[MAGIC_LENGTH / 2],
                          (void*)((uint32_t)(pFwHdrToInstall->HeaderMAC) + SE_MAC_LEN - MAGIC_LENGTH / 2), MAGIC_LENGTH / 2);
...
static SFU_ErrorStatus SwapFirmwareImages(void)
{
  ...
  {
	  /* FULL IMAGE SWAP */
	    /* index_active_slot_partial_end is the index of block (of SLOT_SIZE(SLOT_SWAP) bytes) of first byte following partial
	       image in active slot */
	  index_slot0_partial_end = (SFU_IMG_SLOT_0_REGION_SIZE - TRAILER_SIZE) / SFU_IMG_SWAP_REGION_SIZE;

	    /* offset_block_partial_end is the offset of first byte following partial image, inside index_active_slot_partial_end
	       block */
	    offset_block_partial_end = (SFU_IMG_SLOT_0_REGION_SIZE - TRAILER_SIZE) % SFU_IMG_SWAP_REGION_SIZE;

	    /* index_dwl_slot_partial_end is the index of block (of SLOT_SIZE(SLOT_SWAP) bytes) of first byte following partial
	       image in dwl slot or swap area */
	    ...
	    /* offset_block_final_end is the offset of first byte following final image, inside index_active_slot_final_end block */
	    ...
	    index_slot1_write = (SFU_IMG_SLOT_0_REGION_SIZE - TRAILER_SIZE) / SFU_IMG_SWAP_REGION_SIZE;
...
SFU_IMG_InitStatusTypeDef SFU_IMG_CoreInit(void)
{
  ...
  /*
   * Sanity check: let's make sure the KMS NVM area is under the firewall protection
   */
  ...
  /*
   * Sanity check: let's make sure the KMS NVM area does not overlap Slot 0 area)
   */
  if (!(((REGION_SLOT_0_START) > KMS_DATASTORAGE_END)
    || ((REGION_SLOT_0_END) < (KMS_DATASTORAGE_START))))
  {
    TRACE("\r\n= [FWIMG] KMS NVM area overlaps Slot 0 area\r\n");
    e_ret_status = SFU_IMG_INIT_FLASH_CONSTRAINTS_ERROR;
  }
  /*
   * Sanity check: let's make sure the KMS NVM area does not overlap Slot 1 area)
   */
  if (!(((REGION_SLOT_1_START) > KMS_DATASTORAGE_END)
    || ((REGION_SLOT_1_END) < (KMS_DATASTORAGE_START))))
  {
  ...
  if (!(((REGION_SWAP_START) > KMS_DATASTORAGE_END)
    || ((REGION_SWAP_END) < (KMS_DATASTORAGE_START))))
  {
...
SFU_ErrorStatus SFU_IMG_WriteHeaderValidated(uint8_t *pHeader)
{
  ...
  if (e_ret_status == SFU_SUCCESS)
  {
    ...
    {
      pSrcFWInfoInput = (uint8_t*)(pFwRawHdr->HeaderMAC);
    ...
    {
      /* Store a fingerprint of the header of the FW which did the update to the header of the updated fw */
      /* the original header is in fw_header_validated */
      /* the new header is in FWInfoInput */
