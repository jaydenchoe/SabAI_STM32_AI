...
static void SFU_SCHEME_X509_CORE_PrintHeader(SB_FWHeaderTypeDef *p_FWHeader)
{
  ...
  TRACE("PartialFwOffset : 0x%08x\n\r", p_FWHeader->PartialFwOffset);
  TRACE("PartialFwSize   : 0x%08x\n\r", p_FWHeader->PartialFwSize);
  TRACE("FW Tag          : ");
  ...
  TRACE("\n\r");
  TRACE("PartialFwTag    : ");
  for (int32_t i = 0; i < sizeof(p_FWHeader->PartialFwTag); i++)
  {
    TRACE("%02X", p_FWHeader->PartialFwTag[i]);
  }
  TRACE("\n\r");
  TRACE("Header Signature: ");
  for (int32_t i = 0; i < sizeof(p_FWHeader->HeaderMAC); i++)
  {
    TRACE("%02X", p_FWHeader->HeaderMAC[i]);
    if (!((i + 1) % 32))
    {
      TRACE("\n\r                  ");
    }
...
/**
  ...
  * @param  size_r     size in bytes of the r coordingate
  * @param  p_s          pointer to the s coordinate of the ECDSA signature
  * @param  size_s     size in bytes of the s coordingate
  ...
  *                    > 0 size of der encoded data if successfull and der data
  *                       is produced
  */
                                                    ...
                                                    uint32_t uHashBufferSize)
{
  ...
  if (MessageDigestLength != uHashBufferSize)
  {
    return SFU_ERROR;
  }
#else
  ...
  if(SFU_SUCCESS == ret)
  {
    if(0 != mbedtls_sha256_update_ret(&sha256_ctx, (const unsigned char *) p_Buffer, uBufferSize))
    {
      ret == SFU_ERROR;
    }
  }
  if(SFU_SUCCESS == ret)
  {
    if(0 != mbedtls_sha256_finish_ret(&sha256_ctx, p_HashBuffer))
    {
      memset(p_HashBuffer, 0x00, uHashBufferSize);
      ret == SFU_ERROR;
    }
...
SFU_ErrorStatus SFU_SCHEME_X509_CORE_Init()
{
  ...
  e_ret_status = SFU_SCHEME_X509_CRT_GetSECert(xP11Session, pxP11FunctionList, \
                                               (uint8_t *) P11_OBJ_LABEL_SBSFU_ROOT_CA_CRT, &p_CertChain_RootCA);
  ...
  e_ret_status = SFU_SCHEME_X509_CRT_GetSECert(xP11Session, pxP11FunctionList, \
                                               (uint8_t *) P11_OBJ_LABEL_OEM_INTM_CA_CRT, &p_CertChain_OEM);
  if (SFU_SUCCESS != e_ret_status)
  {
#if defined(SFU_X509_VERBOSE_DEBUG_MODE)
                                                    ...
                                                    uint8_t *p_CertChain_RootCA)
{
  ...
  /*
   * check header magic is there, otherwise this is not a header
   */
   ...
   */
  FWHeader_Size = ((uint32_t) p_FWHeader->HeaderMAC) - ((uint32_t) p_FWHeader);
  ...
  int32_t Asn1Size = EcdsaSigRawToDer(&p_FWHeader->HeaderMAC[0], sizeof(p_FWHeader->HeaderMAC) / 2, \
                                      &p_FWHeader->HeaderMAC[SE_MAC_LEN / 2], sizeof(p_FWHeader->HeaderMAC) / 2, \
