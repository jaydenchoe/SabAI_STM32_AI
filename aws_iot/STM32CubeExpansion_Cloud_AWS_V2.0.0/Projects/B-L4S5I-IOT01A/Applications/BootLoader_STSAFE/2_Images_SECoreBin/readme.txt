...
SE Core binary exports a single entry point to SE CallGate function and a startup
method for data initialization. It is based on the Secure Engine Middleware.

Generated SE Core Binary embeds Key Management Services accessible through this same
single entry point using APIs defined by tKMS interface.
STSAFE-A1xx is configured as an external token to KMS providing additional or alternate services for:
 - ECDSA signature generation
 - ECDSA signature verification
 - Certificate storage
 - ECDSA key pair generation
 - ECDH Diffie-Hellman key derivation
 - Random number generation

It has to be compiled before SBSFU project and requires the execution of STSAFE Provisioning project
for data provisioning and device pairing.

One cryptographic scheme is provided as example to illustrate the cryptographic operations. This cryptographic
scheme uses Certificates (X509) and asymmetric (ECDSA) cryptography.
...
@par Directory contents
   ...
   - 2_Images_SECoreBin/Src/kms_ext_token_stsafe.c              Key Management Services module, STSAFE-A1xx external token plug-in
   ...
   - 2_Images_SECoreBin/Src/stsafe_platf_objects.c              STSAFE-A1xx platform objects methods
   - 2_Images_SECoreBin/Src/stsafea_crypto_stlib_interface.c    Implementation of STSAFE-A1xx Middleware crypto interface
   - 2_Images_SECoreBin/Src/stsafea_service_interface.c         Implementation of STSAFE-A1xx Middleware service interface
   - 2_Images_SECoreBin/Inc/kms_config.h                        Key Management Services configuration file
   - 2_Images_SECoreBin/Inc/kms_ext_token.h                     Header file for KMS STSAFE-A1xx external token plug-in
   ...
   - 2_Images_SECoreBin/Inc/stsafe_platf_objects.h              Header file for STSAFE-A1xx platform objects
   - 2_Images_SECoreBin/Inc/stsafe_platf_objects_config.h       Header file for STSAFE-A1xx platform objects embedded objects defintions
   - 2_Images_SECoreBin/Inc/stsafea_conf.h                      STSAFE-A1xx Middleware configuration file
   - 2_Images_SECoreBin/Inc/stsafea_interface_conf.h            STSAFE-A1xx Middleware interface configuration
   - 2_Images_SECoreBin/Binary/ECCKEY.txt                       Private ECCDSA key for signature verification
...
This preliminary processing is in charge of:
 ...
 - generating the appropriate STSAFE-A1xx pairing keys ("se_keys.s" file)
 - generating the appropriate keys in <IDE folder>/kms_platf_objects_config.h (from injection of Blob importation keys into the KMS embedded objects pattern file)
 - generating the appropriate script to prepare the firmware image ("postbuild.bat") when building the UserApp project
A known limitation of this integration occurs when you update a cryptographic parameter (for instance the cryptographic key).
