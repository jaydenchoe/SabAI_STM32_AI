...
#elif (SECBOOT_CRYPTO_SCHEME == SECBOOT_X509_ECDSA_WITHOUT_ENCRYPT_SHA256)
/* ECDSA Scheme based on X509 certificates, sha256 and no image encryption */
#define SE_IV_LEN               ((int32_t) 16)  /*!< Secure Engine IV Length (Bytes): same size as an AES block*/
#define SE_TAG_LEN              ((int32_t) 32)  /*!< Secure Engine Tag Length (Bytes): SHA-256 for the FW tag */
...
#elif (SECBOOT_CRYPTO_SCHEME == SECBOOT_X509_ECDSA_WITHOUT_ENCRYPT_SHA256)
#define SE_FW_HEADER_METADATA_LEN  (SE_FW_HEADER_TOT_LEN-(44+2048-192-3*32+SE_MAC_LEN))
#endif /* SECBOOT_CRYPTO_SCHEME */
/**
  * @}
  */
...
typedef struct
{
  ...
  uint8_t  PartialFwTag[SE_TAG_LEN];/*!< Partial firmware Tag */
  uint8_t  Reserved[44];           /*!< Reserved for future use: 44 extra bytes to have a header size of 192 bytes*/
  uint8_t  Certificates[2048 - 192 - 3 * 32 - 32 - 3*32]; /*2k - FwImageState - UpdateSourceFingerprint - valid bytes */
  uint8_t  HeaderMAC[SE_MAC_LEN];  /*!< MAC of the full header message */
  uint8_t  FwImageState[3][32];    /*!< Firmware image state - see SE_FwStateTypeDef below for details */
  uint8_t  UpdateSourceFingerprint[32]; /*!< Fingerprint of previous FW header (if this is an update, else 32*0x00) */
} SE_FwRawHeaderTypeDef;
...
typedef enum
{
  FWIMG_STATE_INVALID  = 0, /* image is not valid (typically selftest has failed) and should not boot */
  FWIMG_STATE_VALID    = 1, /* image is valid (selftest passed), image OK to boot if signature checks pass */
  FWIMG_STATE_SELFTEST = 2, /* image under test - self tests to be executed, image OK to boot one time if signature checks pass */
  FWIMG_STATE_NEW      = 3, /* new image, has never run on the device */
  FWIMG_STATE_UNKNOWN  = 4  /* image state is unknown (corrupt), image should not boot */
} SE_FwStateTypeDef;
