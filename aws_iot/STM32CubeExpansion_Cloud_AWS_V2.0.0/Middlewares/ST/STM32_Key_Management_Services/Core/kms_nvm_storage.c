...
/**
  ...
  *
  * @param[in] block         the block identifier
  * @param[in] hdr           pointer to the data header
  *
                              ...
                              nvms_data_header_t *whdrp)
{
  ...
  size_t size = rhdrp->fields.data_size;
  uint8_t *next = whdrp->hdr8;
  uint8_t *free = &next[(((sizeof(nvms_data_header_t) + size - 1UL) |
                          (NVMS_LL_PAGE_SIZE - 1UL)) + 1UL)];
  ...
  */
  result = NVMS_LL_Write((const uint8_t *)&hdr, next, sizeof(nvms_data_header_t));
#else /* 0 */
  ...
  result = NVMS_LL_Write(rhdrp->hdr8 + sizeof(nvms_data_header_t),
                         &next[sizeof(nvms_data_header_t)],
                         size);
  if (result)
  {
    return NVMS_FLASH_FAILURE;
...
static nvms_error_t validate(nvms_block_t block, uint32_t instance)
{
  ...
  Rationale : writing the erased_default value on some platform prevent any further update of the flash
  */
  result = NVMS_LL_Write(hdr.hdr8, dp, sizeof(nvms_data_header_t));
#else /* 0 */
...
/**
  ...
  * @param[in] instance       instance number to attrubute to the data
  *
  * @return                    The operation status.
  * @retval NVMS_NOERROR       if the operation has been successfully completed.
