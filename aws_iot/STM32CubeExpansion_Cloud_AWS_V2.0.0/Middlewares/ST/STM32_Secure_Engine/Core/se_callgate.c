...
/** @defgroup SE_CALLGATE_Private_Constants Private Constants
  * @{
  */
#define TEMP_IRQ_MGMT_DISABLE /* IRQs must be disabled when a secure service is running */

#if defined(__GNUC__)
void SE_ExitHandler_Service(void);
void SE_ExitHandler_Service(void)
{
	/* do nothing as we do not support IRQs when a secure service is running */
}
#endif /* __GNUC__ */
...
/* Application Acive Stack Pointer: 0 means MSP, 1 means PSP */
uint32_t AppliActiveSpMode;
...
SE_ErrorStatus SE_CallGateService(SE_FunctionIDTypeDef eID, SE_StatusTypeDef *peSE_Status, va_list arguments)
{
  ...
  switch (eID)
  {
    ...
    case SE_IMG_GET_FW_STATE:
    {
      ...
      /* Read state multiple times to avoid glitch attack
       */
      e_ret_status = SE_SUCCESS;
      for(int i=0; i<3; i++)
      {
      ...
      if(SE_SUCCESS == e_ret_status)
      {
        ...
        {
          /* 3 readings are the same, return the value */
          *p_FwState = eTempFwState[0];
        }
        else
        {
          /* mismatch in 3 readings, state is not trusted - unknown */
