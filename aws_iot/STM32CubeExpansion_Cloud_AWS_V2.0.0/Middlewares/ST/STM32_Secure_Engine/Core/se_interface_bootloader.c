...
SE_ErrorStatus SE_ExtFlash_Decrypt_Init(SE_StatusTypeDef *pSE_Status, SE_FwRawHeaderTypeDef *pxSE_Metadata)
{
  ...
  else
  {
    ...
    SE_EnterSecureMode();
    e_ret_status = (*SE_CallGatePtr)(SE_EXTFLASH_DECRYPT_INIT, pSE_Status, pxSE_Metadata);
    SE_ExitSecureMode();
#ifdef SFU_ISOLATE_SE_WITH_MPU
  }
#endif /* SFU_ISOLATE_SE_WITH_MPU */
...
SE_ErrorStatus SE_Decrypt_Finish(SE_StatusTypeDef *peSE_Status, uint8_t *pOutputBuffer, int32_t *pOutputSize)
{
  ...
  else
  {
    ...
    /*Secure Engine Call*/
    e_ret_status = (*SE_CallGatePtr)(SE_CRYPTO_LL_DECRYPT_FINISH_ID, peSE_Status, primask_bit, pOutputBuffer, pOutputSize);
                                      ...
                                      int32_t SE_FwType)
{
  ...
  else
  {
    ...
    /*Secure Engine Call*/
    e_ret_status = (*SE_CallGatePtr)(SE_CRYPTO_LL_AUTHENTICATE_FW_INIT_ID, peSE_Status, primask_bit, pxSE_Metadata, SE_FwType);
                                        ...
                                        uint8_t *pOutputBuffer, int32_t *pOutputSize)
{
  ...
  else
  {
    ...
    /*Secure Engine Call*/
    e_ret_status = (*SE_CallGatePtr)(SE_CRYPTO_LL_AUTHENTICATE_FW_APPEND_ID, peSE_Status, primask_bit, pInputBuffer, InputSize,
                                     pOutputBuffer, pOutputSize);
...
SE_ErrorStatus SE_AuthenticateFW_Finish(SE_StatusTypeDef *peSE_Status, uint8_t *pOutputBuffer, int32_t *pOutputSize)
{
  ...
  else
  {
    ...
    /*Secure Engine Call*/
    e_ret_status = (*SE_CallGatePtr)(SE_CRYPTO_LL_AUTHENTICATE_FW_FINISH_ID, peSE_Status, primask_bit, pOutputBuffer, pOutputSize);
...
__root SE_ErrorStatus SE_SFU_IMG_SetActiveFwState(SE_StatusTypeDef *peSE_Status, SE_FwStateTypeDef* p_FwState)
{
  ...
  if (0 != SE_IsUnprivileged())
  {
    SE_FwStateTypeDef params[1] = {(SE_FwStateTypeDef)p_FwState};
