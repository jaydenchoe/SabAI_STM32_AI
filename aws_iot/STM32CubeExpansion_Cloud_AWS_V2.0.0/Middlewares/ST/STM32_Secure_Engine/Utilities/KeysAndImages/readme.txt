...
If you face issues installing the modules, please check your python version. Python3 might be required for some modules.
...
By default, windows users do not need to install the python modules as windows executables are delivered and used by default by the scripts.
...
=================================
'bigelftuner.py' functionalities
=================================
This script is dedicated to the CubeIDE environment.
It aims at changing the entry point of the single elf file (SBSFU + application),
to allow debugging easily with CubeIDE.
This script relies on the lief module, so python3 may be required.
Example for Ubuntu 18.04 users
> pip3 install lief

To rebuild the windows executable, please do:
>pyinstaller --onefile bigelftuner.py
>cp -p dist/bigelftuner.exe win/