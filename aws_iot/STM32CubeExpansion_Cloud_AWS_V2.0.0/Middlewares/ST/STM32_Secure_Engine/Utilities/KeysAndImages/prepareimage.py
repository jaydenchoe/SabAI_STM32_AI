...
def do_header_lib(args):
    ...
    # Pad, leaving 64 bytes for signature + 3*32 bytes for valid info + 3*32 bytes for image state
    if (args.cert_fw_leaf or args.cert_fw_inter):
        if 'pack' in args.subcmd:
            padding = args.offset - (len(header) + 3*32 + 32 + 64 + 3*32)
            imagestate = "new"
        else:
            padding = args.offset - (len(header) + 3*32 + 32 + 64 + 3*32)
            imagestate = "valid"
...
def do_merge(args):
    ...
    if (beginaddress_header < address_just_after_sbsfu):
        ...
        if args.loader:
            ...
            big_binary = sbsfu_binary + padd_before_header * pack("B",args.value) + header_binary
        else:
            padd_before_header =   beginaddress_header - address_just_after_sbsfu
